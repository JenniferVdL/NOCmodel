...
from functools import reduce #compute the product of a list used in MatchKans
...
colS = [i for i,x in enumerate(headers) if x.split("#")[0] == "Size"]
for lines in file:
    ...
    for alleleNumber, colNumber in enumerate(colA):
        ...
        data[sample][locus][line[colNumber]]["size"] = float(line[colS[alleleNumber]])
file.close()


#AlleleFrequency
freqs={}
AFfile = open("AlleleFrequencies.txt", "r") #Change AlleleFrequenciesFile.
AFfile.readline()
totalAinF = 0
for lines in AFfile:
...
for sample in allelecount:
    ...
    Features[sample]["MinNOC"] = round(float(min(allelecount[sample]))/float(2))
...
for sample in Features:
    for loci in lociMarkers:
        if loci in AlFreqs[sample]:
            ...
            Features[sample]["Locus"][loci]["PercAF"+"_"+loci] = len(AlFreqs[sample][loci])/float(totalAinF)*100
        else:
...
nocFile = open("nocs.txt","r")

for lines in nocFile:
    line = lines.rstrip().split("\t")
    sample = line[0]
    NOC = line[1]
    if sample in Features:
        Features[sample]["NOC"] = NOC
...
scaler = joblib.load("scaler.pkl") #change
model = joblib.load("model.pkl") #change

featuresModel = ['TAC', 'stdAllele', 'MAC5-6'] #change

def gettingData(FEATURES):
    sampleNames = []
    sample_ordered_features = [] 
    for sample, items in FeatureSet.items():
        sampleList = []
        ...
        newSampleList = sampleList + ordered.values() #locus
        sample_ordered_features.append(map(float, newSampleList)) # locus
    sample_features = np.array(sample_ordered_features)
    sample_features=scaler.transform(sample_features)
    return sample_features, sampleNames

sample_features, sampleNames = gettingData(featuresModel)

prediction = model.predict(sample_features)

print("Sample\tPredicted NOC\tProbability")
for i in range(len(sampleNames)):
    print(str(sampleNames[i])+"\t"+str(prediction[i])+"\t"+str(model.predict_proba(sample_features)[i][prediction[i]-1]))
